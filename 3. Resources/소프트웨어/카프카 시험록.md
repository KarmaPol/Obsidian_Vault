...
```bash
# 워커 노드
docker swarm join --token SWMTKN-1-573jtcnkprooeuj7fo61lk2wyekefclu8rcpboyly30wuf0xmq-6brd9fq66p8dw32gc9l64z3qt 43.202.238.236:2377 --advertise-addr 3.34.180.0
```
   - 서버 라벨 등록
```bash
...
- 도커 이미지 빌드
```bash
docker build --platform linux/amd64 -t kafka-test:0.1 .
```
